...
def get_kospi200():
    """KOSPI 200 구성종목 조회 (KRX API → FDR 순서)"""
    # 1순위: KRX 직접 API (SSL 충돌 없음)
    try:
        ...
        _r    = _req.post(_url, data=_payload, headers=_hdr, timeout=15)
        _rows = _r.json().get('OutBlock_1', [])
        if len(_rows) >= 100:
            result = {}
            for it in _rows:
                code = str(it.get('ISU_SRT_CD', '')).zfill(6)
                name = str(it.get('ISU_ABBRV', ''))
                if code and name:
                    result[code + '.KS'] = name
            return result
    except Exception:
